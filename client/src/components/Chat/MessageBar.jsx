 ...
 const photoPickerChange =async(e)=>{
   try {
    ...
    if(response.status === 201){
      ...
      dispatch({
        type:reducerCases.ADD_MESSAGE , newMessage:{...response.data.message} ,fromSelf:true
      })
    }
  ...
  const sendMessage = async()=>{
    try {
      ...
      dispatch({
        type:reducerCases.ADD_MESSAGE , newMessage:{...data.message} ,fromSelf:true
      })
  ...
  return <div className=" bg-panel-header-background h-20 px-4 flex items-center gap-6 relative">
    ...
    <div className=" flex gap-6">
      ...
      title="Emoji" id ="emoji-open"
      onClick={handleEmojiModal}
      />
      {
        showEmojiPicker && <div className="absolute bottom-20 z-40" ref={emojiPickerRef}>
    ...
    <div className=" w-full rounded-lg h-10 flex items-center">
      ...
      value={message}
      onKeyDown={(e)=>enterMessage(e)} //Enter has to be place in input
      />
    </div>
